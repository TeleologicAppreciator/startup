...
- [X] **Stores data in MongoDB** - I connected my backend service to a live MongoDB Atlas cluster and migrated all game data into the database. Player portfolios, holdings, funds, and profits are now stored in a players collection. Whenever a user buys stock, updates their funds, or when the daily cycle resets portfolios, the backend updates the corresponding MongoDB documents. Leaderboard data is dynamically generated by querying MongoDB and sorting players by their profit field, ensuring the leaderboard always reflects the latest stored data.
- [X] **Stores credentials in MongoDB** - I implemented full authentication storage in MongoDB using a users collection. When new users register, their password is hashed with bcrypt and stored securely in the database, along with a persistent session token used for cookie-based authentication. Login, logout, and token validation all interact directly with MongoDB, replacing the temporary mock authentication system from earlier deliverables. This ensures persistent, secure user accounts across sessions and devices.
